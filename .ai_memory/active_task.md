...
## Current Focus
**Phase 14 — Security Hardening COMPLETE** (`feature/p14-security`, commit + push pending — the previous session died before syncing memory; all work was on disk and has been re-verified live in this session). Verified green: pytest **52 passed** (35 app + 17 lambda), ruff clean, shellcheck clean, **41 script tests** (incl. 8 new security tests), terraform validate + fmt clean. Live on Floci: `scripts/security.sh all` demos pass (KMS round trip, Secrets Manager masked read, Cognito full auth flow with real JWT claims, WAF rules, IAM scoped policy) + `scripts/security-audit.sh` clean (secret scan + both IAM users scoped). All 10 `module.security` resources in terraform state. ADR-12 written; memory synced. **Phase 14 FULLY SHIPPED — PR #1 squash-merged to main as `245516e`, branch deleted (local + remote). CI green after fixing 3 real issues the first GitHub run caught (hermetic observability tests, gitleaks fetch-depth 0, pip stripped from runtime image for trivy, buildx load: true). Next: Phase 15 — Reliability.**
...
- (none) — main clean at `245516e`; Phase 15 next.
