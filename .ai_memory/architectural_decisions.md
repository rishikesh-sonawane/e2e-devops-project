...
## ADR 13: Phase 15 Reliability — Backup/Restore Drills, Failure Injection, and the Auto-Scaling Reconciler (with Honest Floci Limits)
- **Status:** Accepted
- **Decision:** Phase 15 reliability = **`scripts/reliability.sh`** with six commands: `backup` (paginated DynamoDB export + S3 sync → `data/backups/cloud-<ts>/` with manifest), `restore` (batch-write-item, 25/chunk with UnprocessedItems retries + s3 sync back + count verification), `drill` (probe data → backup → simulated loss → restore → verify → **measured RTO**, RPO=0 for the drill), `chaos kill-pod|kill-api|fail-image` (failure injection with recovery: k3s pod deletion → Deployment controller self-heal; API process kill → restart; corrupt upload → Lambda FAILED dead-letter → fix + reset → **replay the S3 event via `aws lambda invoke`** → PROCESSED), `scaling` (live HPA + Deployment reconcilers on k3s), and `reconcile [--apply]` (the **explicit desired-vs-actual reconciler loop** over ASG + Deployment + HPA). Terraform gains `modules/autoscaling` (launch config `ami-test`/`t3.micro` + ASG `imageflow-asg` min 1/max 3/desired 1). Docs: `docs/reliability.md`. Tests: `scripts/tests/test_reliability.sh` (15, stateful Python fake aws + fake kubectl).
- **Context:** docs/roadmap.md Phase 15 demands availability/scalability/resiliency/fault-tolerance/DR on Floci. The probes decided the design: what is *live* vs *stored-only* on Floci (below).
- **Probe findings (Floci 0.2.0 / server 1.5.34):** ① **Autoscaling is LIVE when backed by a launch template** — `aws_launch_configuration` resources FAIL on Floci (create returns success but describe returns empty → Terraform post-create lookup errors with "empty result"), so the module uses a launch template instead; an ASG backed by a launch template **genuinely launches EC2 instances and reconciles replacements** (probe-verified live: terminated `i-…a233` → new `i-…4c1c` running, `chaos kill-instance` reproduces it in ~9s). One quirk: the replacement stays `Pending` in the ASG's own view while the EC2 instance reports `running`; ② DynamoDB scan pagination (`--max-items`/`--starting-token`), `batch-write-item` (incl. UnprocessedItems retry), `s3 sync` both directions, and `aws lambda invoke` all work → the backup/restore drill and the Lambda retry path are **fully live**, not simulated; ③ deleting a pod genuinely exercises the k3s Deployment controller; killing the API process genuinely requires restart (and uvicorn's graceful SIGTERM shutdown can answer one more /health — the script polls for failure).
- **Design decisions:** the drill creates a throwaway 1×1 PNG + metadata record so recovery is *verified* (record + object back), not assumed; restore verifies counts against the manifest and fails loudly on drift; `reconcile` is dry-run by default (`--apply` corrects via `update-auto-scaling-group` / `kubectl scale`) so drift reporting is safe to run anywhere; the fail-image retry path replays the S3 event via `aws lambda invoke` (no venv/handler dependency in the script — keeps tests hermetic).
- **Consequences:** a complete, demonstrable reliability story for $0 — live backup/restore with measured RTO, three real failure-injection drills, and an explicit reconciler loop that makes the desired-vs-actual pattern visible and testable. Honest limits (ASG instance launch is config-only on Floci) documented; the ASG stays real-AWS-correct for a live account.

## ADR 12: Phase 14 Security Surface — Secrets-Backed Creds, Cognito, WAF, Least-Privilege IAM (with Honest Floci Limits)
- **Status:** Accepted
- **Decision:** Phase 14 security = **KMS key + alias, Secrets Manager secret (`imageflow/app-secret`), Cognito user pool/client/demo user, WAF v2 web ACL (rate-limit + AWS-managed common rules), and a least-privilege IAM demo user** — provisioned by the Terraform `security` module (new `random` provider for the generated demo password). The API can source its AWS credentials from Secrets Manager (`IMAGEFLOW_SECRET_NAME`, `resolve_aws_credentials()` in `app/services/secrets.py`) with a **non-fatal fallback** to env/settings. CI gains supply-chain gates: pip-audit (both requirement files) + gitleaks + trivy fs (security job) and trivy image (build job). Lambda IAM tightened: `logs:CreateLogStream`/`PutLogEvents` scoped to the function's log-group ARN.
- **Context:** docs/roadmap.md Phase 14 demanded identity, secrets, encryption, least privilege, and supply-chain scanning. Floci 0.2.0 supports Secrets Manager, KMS, Cognito, and WAF v2 — all probe-verified before committing to the design.
- **Probe findings (Floci 0.2.0 / server 1.5.34):** ① **Cognito is faithful** — `admin-create-user` → `NEW_PASSWORD_REQUIRED` challenge → `admin-respond-to-auth-challenge` returns a **real JWT** with verifiable claims (`iss`=`http://localhost:4566/us-east-1_<id>`, `sub`, `exp`, `iat`, `cognito:username`, `email`); ② **IAM authorization is NOT enforced** — Floci validates SigV4 signatures but a user with a one-bucket read-only policy can still `ListBuckets` (probe-verified). Policy *design* is real-AWS-correct; enforcement is a live-account control (same honest posture as the Phase 12 CodeDeploy simulation caveat); ③ **WAF v2 ACLs + rules persist** and are inspectable via `get-web-acl`, but there is no ALB/API-Gateway to attach them to locally — the stored-ACL + inspection story is what's demonstrable; ④ Secrets Manager `create/put/get` full JSON SecretString round trip works.
- **Security-posture decisions:** the demo user's temporary Cognito password is a **generated `random_password`** (a literal in the repo would be flagged by the audit — it already caught a fake key in a test fixture); `security.sh` never echoes secret *values*, only key names / opaque ciphertext; the secret scan in `scripts/security-audit.sh` excludes `data/`, `.venv/`, lockfiles, and the private `component-wise-architecture/` notes.
- **Consequences:** a complete, demonstrable security story for $0 — live demos in `scripts/security.sh` (kms · secrets · cognito · waf · iam), audit in `scripts/security-audit.sh`, write-up in `docs/security.md` with a threat-model → control map. Honest limits (IAM enforcement, WAF attachment, ACM issuance, CloudTrail delivery) documented as real-account controls.
