...
>
> **Full-system live drill (2026-08-05, PR #4 `eb20ed1`):** running every layer together on Floci caught four real issues, all fixed: ① `chaos kill-api` killed the wrong pid on macOS (backgrounded `cd && nohup uvicorn` makes `$!` the wrapper pid — off-by-one) → `deploy.sh` now resolves the real port listener (`lsof`, `pgrep` fallback) + regression test; ② fail-image uploaded corrupt bytes before the DDB record (S3 event could fire while the record didn't exist → Lambda skipped → stuck PENDING forever) → record-first; ③ the API route had the same latent race → reversed via shared deterministic `storage.original_key()`, **plus rollback** (`delete_record` on upload failure — no zombie PENDING items); ④ Terraform perpetual in-place diffs from Floci normalization quirks (alarm `datapoints_to_alarm`, Cognito pool defaults, IAM user tags) → documented `ignore_changes`, plan now idempotent. Reproduce the whole system by hand: `docs/manual-verification.md`.
